...
//         } else {
//           assert(false &&
//                  "Invalid exponent closing token found, must be 2e+2 2e-2
//                  2e2");
//         }
//       }
//     }
...
char *parse_string(Parser *p) {
  ...
  while (1) {
    ...
    const int current_token = current_token_data;
...
void deserialize_create_table(Parser *p) {
  char *current_token = get_token_at(p->lexer, p->counter);
  if (!current_token) {
    goto deserialization_failed;
  }
  if (strcmp(current_token, "CREATE")) {
    goto deserialization_failed;
  }
  p->counter++;
  int token_counter = 1;
  int last_valid_token = -1;
  int name_type_completed = 0;
  while (1) {
    current_token = get_token_at(p->lexer, p->counter);
    if (!current_token) {
      goto deserialization_failed;
    }
    if (token_counter == 1 && !strcmp(current_token, "TABLE")) {
      token_counter++;
    } else if (token_counter == 2) {
      token_counter++;
    } else if (token_counter == 3 && !strcmp(current_token, "(")) {
      token_counter++;
    }
    p->counter++;
  }
deserialization_failed:
  assert(false && "failed to terminate deserialize_create_table correctly");
}
void deserialize_entry(Parser *p) {
  u_short current_token = get_token_at_cast(p->lexer, 0);
  switch (current_token) {
  case LOWER_C: {
    deserialize_create_table(p);
    break;
  }
  case LOWER_D: {
    break;
